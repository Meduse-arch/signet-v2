...
export function Taskbar({ onOpenMenu, pinnedActions, activeIds }: TaskbarProps) {
  const systemActions = pinnedActions.filter(a => a.modId.startsWith('system-'));
  const classicActions = pinnedActions.filter(a => !a.modId.startsWith('system-'));
  ...
  return (
    <div className="absolute bottom-0 left-0 w-full z-40 pointer-events-none flex justify-center pb-6">
      <div className="pointer-events-auto flex items-end gap-6 px-8 py-2">
        {/* System actions on the left */}
        {systemActions.map(renderAction)}

        {systemActions.length > 0 && <div className="w-1 h-1 rounded-full bg-white/10 mx-2 mb-4" />}

        {/* Main Signet Button (Menu) in the center */}
        <button 
          onClick={(e) => {
            e.stopPropagation();
            onOpenMenu();
        ...
        {classicActions.length > 0 && <div className="w-1 h-1 rounded-full bg-white/10 mx-2 mb-4" />}

        {/* Classic actions on the right */}
        {classicActions.map(renderAction)}
      </div>
    </div>
  );
}
